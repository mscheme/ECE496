...
vector<double> partAreas(Mat save_img);
...
static DWORD WINAPI CaptureThread(LPVOID ThreadPointer){
	...
	strArray = partNames();
	areaArray = partAreas(*(Instance->Temp_back));

	//set p1, p2, p3, p4, p5 = areaArray[0-4]

	printf("p1 range: %.2f -%.2f\n", strArray[0], (p1*(1-per)), (p1*(1+per)));
	printf("p2 range: %.2f -%.2f\n", strArray[1], (p2*(1-per)), (p2*(1+per)));
	printf("p3 range: %.2f -%.2f\n", strArray[2], (p3*(1-per)), (p3*(1+per)));
	printf("p4 range: %.2f -%.2f\n", strArray[3], (p4*(1-per)), (p4*(1+per)));
	printf("p5 range: %.2f -%.2f\n", strArray[4], (p5*(1-per)), (p5*(1+per)));
...
vector<double> partAreas(Mat save_img)
{
	...
	//place part
	//take stationary image
	//background subtraction, dilate, erode, canny edge, contours, find area
	//output area
	//verify value
