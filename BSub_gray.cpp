...
static DWORD WINAPI CaptureThread(LPVOID ThreadPointer){
	...
	int FramerCounter=0;
	Mat Temp =Mat(Instance->Frame->rows,Instance->Frame->cols,CV_8UC4),Temp2, Temp3, Gray_back;
	...
	while(1){
		...
		//convert temp to gray scale save in temp2
		cv::cvtColor(Temp, Temp2, CV_RGBA2GRAY);
		cv::cvtColor(*(Instance->Temp_back), Gray_back, CV_RGBA2GRAY);

		//background subtraction saved image-current image
		Temp3 = Gray_back - Temp2;

		//threshold
		//printf("%d", Instance->Threshold);
		Instance->Threshold = -200;
		threshold(Temp3, *(Instance->Frame), Instance->Threshold, 0,CV_THRESH_TOZERO);
