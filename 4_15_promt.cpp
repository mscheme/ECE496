...
vector<double> partAreas(Mat save_img, vector<String> strArray, int rows, int cols);
...
static DWORD WINAPI CaptureThread(LPVOID ThreadPointer){
	...
	*/
	...
	strArray = partNames();
//	areaArray = partAreas(*(Instance->Temp_back), strArray, rows, cols);
	...
	printf("p1 range: %.2f -%.2f\n", (p1*(1-per)), (p1*(1+per)));
	cout<<strArray[1]<<endl;
	printf("p2 range: %.2f -%.2f\n", (p2*(1-per)), (p2*(1+per)));
	cout<<strArray[2]<<endl;
	printf("p3 range: %.2f -%.2f\n",  (p3*(1-per)), (p3*(1+per)));
	cout<<strArray[3]<<endl;
	printf("p4 range: %.2f -%.2f\n", (p4*(1-per)), (p4*(1+per)));
	cout<<strArray[4]<<endl;
	printf("p5 range: %.2f -%.2f\n", (p5*(1-per)), (p5*(1+per)));
...
vector<double> partAreas(Mat save_img, vector<String> strArray, int rows, int cols)
{
	...
	Rect roi(180, 0, 300, 330);
	Mat image_roi;
	CAMERA_AND_FRAME *Temp_Inst;
	Mat Temp =Mat(rows, cols,CV_8UC4), temp2, temp3, temp4, temp5;

	cout<<"Place part 1: " << strArray[0] <<" on belt" <<endl;
	while (ready == 0)
    {
	...
	CLEyeCameraGetFrame(Temp_Inst->CameraInstance,Temp.data);
	//Image code start
	temp2 = save_img- Temp;
